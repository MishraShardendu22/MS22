...
import { Search, Award, ExternalLink, ChevronLeft, ChevronRight, ArrowUpRight, Filter, X, CheckCircle, Calendar, Shield } from 'lucide-react'
...
export default function CertificatesPage() {
  ...
  const fetchCertificates = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await certificatesAPI.getAllCertificates(currentPage, limit)
        ...
        <div className="container mx-auto px-4 py-20 md:py-28 relative">
          <div className="max-w-5xl mx-auto">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tight animate-fadeInUp">
              Certifications & <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 animate-gradient">Awards</span>
            </h1>
            <p className="text-gray-400 text-xl md:text-2xl max-w-3xl leading-relaxed animate-fadeInUp animation-delay-200">
              Verified certifications and professional credentials from leading technology organizations
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
            ...
            <div className="relative group animate-fadeIn">
              ...
              <div className="relative p-8 bg-gray-900/90 backdrop-blur-xl border border-gray-800/50 rounded-2xl space-y-5 shadow-2xl">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold text-white">Filter by Skill</h3>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
            <div className="flex items-center gap-3 px-5 py-3 bg-gray-900/50 backdrop-blur-xl border border-gray-800/50 rounded-xl">
              <div className="flex items-center gap-2">
                ...
                <span className="text-cyan-400 font-bold text-lg">{filteredCertificates.length}</span>
            ...
            {selectedSkills.length > 0 && (
              <div className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/30 rounded-xl">
                ...
                <span className="text-cyan-400 font-semibold">
                  Filtered by {selectedSkills.length} {selectedSkills.length !== 1 ? 'skills' : 'skill'}
                </span>
              </div>
            )}
          </div>
        ...
        {filteredCertificates.length === 0 ? (
          <div className="max-w-7xl mx-auto text-center py-32">
            ...
            <h3 className="text-3xl font-bold text-white mb-4">No certificates found</h3>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 min-h-[600px]">
            {paginatedCertificates.map((certificate, index) => (
              <CertificateCard key={certificate._id || certificate.inline?.id || index} certificate={certificate} index={index} />
            ))}
          </div>
        )}
            ...
            <div className="flex-1 mb-5">
              {certificate.issue_date && (
                <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
                  ...
                  <span>Issued: {new Date(certificate.issue_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</span>
                  {certificate.expiry_date && (
                    <span className="text-gray-600">• Expires: {new Date(certificate.expiry_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</span>
                  )}
                </div>
              )}
