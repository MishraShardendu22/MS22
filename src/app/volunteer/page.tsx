...
import { Search, Heart, ExternalLink, ChevronLeft, ChevronRight, ArrowUpRight, Filter, X, MapPin, Calendar } from 'lucide-react'
  ...
  const fetchVolunteers = async () => {
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        throw new Error(response.message || 'Failed to fetch volunteer experiences')
      }
    } catch (err) {
      ...
      <div className="relative border-b border-gray-800/50 z-10">
        ...
        <div className="container mx-auto px-4 py-20 md:py-28 relative">
          <div className="max-w-5xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-500/10 to-purple-500/10 border border-pink-500/20 rounded-full text-pink-400 text-sm font-semibold mb-8 backdrop-blur-sm animate-fadeIn">
              <Heart className="w-4 h-4" />
              <span>Community Impact</span>
            </div>
            <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tight animate-fadeInUp">
              Volunteer <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-purple-500 to-cyan-600 animate-gradient">Experience</span>
            </h1>
            <p className="text-gray-400 text-xl md:text-2xl max-w-3xl leading-relaxed animate-fadeInUp animation-delay-200">
              Explore my volunteer work and contributions to various communities and organizations
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        <div className="max-w-7xl mx-auto mb-10 space-y-6">
          <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 px-6 py-4 rounded-2xl font-semibold transition-all shadow-lg relative group ${
                showFilters || selectedTechs.length > 0
                  ? 'bg-gradient-to-r from-pink-500/20 to-purple-500/20 text-pink-400 border border-pink-500/40 shadow-pink-500/20'
            ...
            >
              ...
              {selectedTechs.length > 0 && (
                <span className="absolute -top-2 -right-2 flex items-center justify-center w-7 h-7 text-xs font-bold bg-gradient-to-br from-pink-500 to-purple-600 text-white rounded-full shadow-lg shadow-pink-500/50 animate-pulse">
          ...
          {showFilters && allTechnologies.length > 0 && (
            <div className="relative group animate-fadeIn">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-cyan-500/20 rounded-2xl blur opacity-50" />
              <div className="relative p-8 bg-gray-900/90 backdrop-blur-xl border border-gray-800/50 rounded-2xl space-y-5 shadow-2xl">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold text-white">Filter by Technology</h3>
                ...
                <div className="flex flex-wrap gap-3">
                  {allTechnologies.map(tech => (
                    <button
                      ...
                      className={`group/tech relative px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-300 ${
                        selectedTechs.includes(tech)
                          ? 'bg-gradient-to-r from-pink-500/20 to-purple-500/20 text-pink-400 border-2 border-pink-500/40 shadow-lg shadow-pink-500/20 scale-105'
                    ...
                    >
                      {selectedTechs.includes(tech) && (
                        <div className="absolute inset-0 bg-gradient-to-r from-pink-500/10 to-purple-500/10 rounded-xl animate-pulse" />
                      )}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
            <div className="flex items-center gap-3 px-5 py-3 bg-gray-900/50 backdrop-blur-xl border border-gray-800/50 rounded-xl">
              <div className="flex items-center gap-2">
                ...
                <span className="text-pink-400 font-bold text-lg">{filteredVolunteers.length}</span>
                <span className="text-gray-400">of</span>
                <span className="text-white font-bold text-lg">{volunteers.length}</span>
                <span className="text-gray-400">volunteer experiences</span>
              </div>
            </div>
            {selectedTechs.length > 0 && (
              <div className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-pink-500/10 to-purple-500/10 border border-pink-500/30 rounded-xl">
                <Filter className="w-4 h-4 text-pink-400" />
                <span className="text-pink-400 font-semibold">
                  Filtered by {selectedTechs.length} {selectedTechs.length !== 1 ? 'technologies' : 'technology'}
                </span>
              </div>
            )}
          </div>
        ...
        {filteredVolunteers.length === 0 ? (
          <div className="max-w-7xl mx-auto text-center py-32">
            ...
            <h3 className="text-3xl font-bold text-white mb-4">No volunteer experiences found</h3>
            ...
            {(searchQuery || selectedTechs.length > 0) && (
              <button
                onClick={clearFilters}
                className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-pink-500/10 to-purple-500/10 text-pink-400 border-2 border-pink-500/30 rounded-xl hover:from-pink-500/20 hover:to-purple-500/20 transition-all font-semibold shadow-lg shadow-pink-500/10"
              >
                <X className="w-5 h-5" />
                Clear all filters
              </button>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 min-h-[600px]">
            {paginatedVolunteers.map((volunteer, index) => (
              <VolunteerCard key={volunteer._id || volunteer.inline?.id || index} volunteer={volunteer} index={index} />
            ))}
          </div>
        )}
                ...
                return (
                  <button
                    ...
                    className={`relative min-w-12 px-5 py-3 rounded-xl font-bold transition-all shadow-lg ${
                      currentPage === page
                        ? 'bg-gradient-to-r from-pink-500 to-purple-600 text-white scale-110 shadow-pink-500/50'
                  ...
                  >
                    {currentPage === page && (
                      <div className="absolute inset-0 bg-gradient-to-r from-pink-500 to-purple-600 rounded-xl blur opacity-50 animate-pulse" />
                    )}
  ...
  return (
    <Link href={`/volunteer/${volunteerId}`}>
      ...
      >
        ...
        <div className="absolute -inset-0.5 bg-gradient-to-r from-pink-500 via-purple-500 to-cyan-600 rounded-2xl blur opacity-0 group-hover:opacity-30 transition duration-500" />
        ...
        <div className="relative h-full bg-gray-900/90 backdrop-blur-xl border border-gray-800/50 rounded-2xl overflow-hidden transition-all duration-500 hover:border-pink-500/50 hover:shadow-2xl hover:shadow-pink-500/20 hover:-translate-y-2 cursor-pointer">
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-pink-500/0 via-purple-500/0 to-cyan-500/0 group-hover:from-pink-500/10 group-hover:via-purple-500/5 group-hover:to-cyan-500/10 transition-all duration-500" />
              ...
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-xl font-bold text-white line-clamp-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-pink-400 group-hover:to-purple-500 transition-all duration-300">
                ...
                <div className="shrink-0 p-2 bg-gray-800/50 rounded-lg group-hover:bg-gradient-to-br group-hover:from-pink-500/20 group-hover:to-purple-500/20 border border-gray-700/50 group-hover:border-pink-500/40 transition-all duration-300">
            ...
            <div className="flex-1 mb-5">
              ...
              {volunteer.start_date && (
                <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
                  ...
                  <span>{volunteer.start_date} - {volunteer.end_date || (volunteer.current ? 'Present' : 'N/A')}</span>
                </div>
              )}
              ...
              {volunteer.technologies && volunteer.technologies.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  ...
                  {volunteer.technologies.length > 4 && (
                    <span className="px-3 py-1.5 text-xs font-semibold bg-gradient-to-r from-pink-500/20 to-purple-500/20 text-pink-400 rounded-lg border border-pink-500/30 group-hover:from-pink-500/30 group-hover:to-purple-500/30 transition-all">
                      +{volunteer.technologies.length - 4}
                    </span>
                  )}
                </div>
