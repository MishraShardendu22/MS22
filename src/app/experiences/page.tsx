...
import { Search, Briefcase, ExternalLink, ChevronLeft, ChevronRight, ArrowUpRight, Filter, X, Building2, Calendar } from 'lucide-react'
...
export default function ExperiencesPage() {
  ...
  const fetchExperiences = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await experiencesAPI.getAllExperiences(currentPage, limit)
      ...
      <div className="relative border-b border-gray-800/50 z-10">
        ...
        <div className="container mx-auto px-4 py-20 md:py-28 relative">
          <div className="max-w-5xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border border-cyan-500/20 rounded-full text-cyan-400 text-sm font-semibold mb-8 backdrop-blur-sm animate-fadeIn">
              <Briefcase className="w-4 h-4" />
              <span>Professional Journey</span>
            </div>
            <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tight animate-fadeInUp">
              Work <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 animate-gradient">Experience</span>
            </h1>
            <p className="text-gray-400 text-xl md:text-2xl max-w-3xl leading-relaxed animate-fadeInUp animation-delay-200">
              Explore my professional journey and career milestones across various organizations
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
            <div className="flex items-center gap-3 px-5 py-3 bg-gray-900/50 backdrop-blur-xl border border-gray-800/50 rounded-xl">
              <div className="flex items-center gap-2">
                ...
                <span className="text-cyan-400 font-bold text-lg">{filteredExperiences.length}</span>
                <span className="text-gray-400">of</span>
                <span className="text-white font-bold text-lg">{experiences.length}</span>
        ...
        {filteredExperiences.length === 0 ? (
          <div className="max-w-7xl mx-auto text-center py-32">
            ...
            <h3 className="text-3xl font-bold text-white mb-4">No experiences found</h3>
            <p className="text-gray-400 text-lg mb-8 max-w-md mx-auto">
              Try adjusting your search terms or filters to discover more experiences
            </p>
            {(searchQuery || selectedTechs.length > 0) && (
              <button
                onClick={clearFilters}
...
function ExperienceCard({ experience, index }: ExperienceCardProps) {
  ...
  const latestPosition = experience.experience_time_line[experience.experience_time_line.length - 1]
          ...
          <div className="relative p-7 flex flex-col h-full">
              ...
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-xl font-bold text-white line-clamp-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-blue-500 transition-all duration-300">
            ...
            <div className="flex-1 mb-5">
              <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
                ...
                <span>{latestPosition.start_date} - {latestPosition.end_date || 'Present'}</span>
              </div>
            ...
            <div className="flex items-center gap-2 pt-5 border-t border-gray-800/50 group-hover:border-gray-700/50 transition-colors">
              ...
              <div className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold bg-gray-800/50 text-gray-400 rounded-lg group-hover:bg-gray-800 group-hover:text-purple-400 transition-all duration-200">
                <Briefcase className="w-4 h-4" />
                <span>{experience.experience_time_line.length} {experience.experience_time_line.length === 1 ? 'Role' : 'Roles'}</span>
              </div>
            </div>
          </div>
        </div>
