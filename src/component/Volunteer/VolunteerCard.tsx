...
const VolunteerCard = ({ volunteer, index }: VolunteerCardProps) => {
  ...
  const startDate = formatDate(volunteer.start_date)
  const endDate = volunteer.current ? 'Present' : formatDate(volunteer.end_date)
      ...
      <div className="relative bg-linear-to-br from-gray-900/50 to-gray-950/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl overflow-hidden transition-all duration-300 hover:border-pink-500/40 hover:shadow-lg hover:shadow-pink-500/10">
        ...
        <div className="relative p-5">
          ...
          <div className="flex items-start justify-between mb-3">
            ...
            {volunteer.current && (
              <span className="px-2.5 py-1 text-xs font-bold bg-pink-500/10 text-pink-400 rounded-md border border-pink-500/30 shrink-0">
                Active
              </span>
            )}
          </div>
          ...
          {volunteer.certificate_link && (
            <div className="flex items-center gap-2 pt-3 border-t border-gray-800/50">
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-gray-800/40 hover:bg-gray-800/60 text-gray-400 hover:text-pink-400 rounded-lg transition-all duration-200 group/icon"
...
export const VolunteerDisplay = () => {
  ...
  const fetchVolunteers = async (page: number) => {
    try {
      console.log('🎯 Fetching volunteers, page:', page)
      if (page === 1) {
        setLoading(true)
      } else {
      ...
      if (response.status === 200 && response.data) {
        const sortedVolunteers = response.data.volunteers.sort((a, b) => a.order - b.order)
        console.log('🎯 Volunteers loaded:', sortedVolunteers.length)
        setVolunteers(sortedVolunteers)
        setTotalVolunteers(response.data.total || sortedVolunteers.length)
      } else {
        console.log('🎯 No volunteers data')
        setVolunteers([])
  ...
  if (!loading && volunteers.length === 0) return null
        ...
        <div className="mb-6 min-h-[600px] relative">
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-opacity duration-300 ${paginationLoading ? 'opacity-0' : 'opacity-100'}`}>
            {volunteers.map((volunteer, index) => (
              <VolunteerCard key={volunteer._id} volunteer={volunteer} index={index} />
            ))}
          </div>
        </div>
