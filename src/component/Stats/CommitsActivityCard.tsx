...
import { ComposedChart, Line, Bar, Area, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, Legend } from 'recharts'
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 shadow-xl">
        <p className="text-xs text-gray-400 mb-1">Week of {new Date(data.week).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
...
export const CommitsActivityCard = ({ commits, calendar }: CommitsActivityCardProps) => {
  ...
  const avgCommits = weeklyData.reduce((sum: number, w: any) => sum + w.commits, 0) / weeklyData.length

  const enrichedData = weeklyData.map((week: any, index: number) => {
    const movingAvg = index >= 3 
      ? weeklyData.slice(Math.max(0, index - 3), index + 1).reduce((sum: number, w: any) => sum + w.commits, 0) / Math.min(4, index + 1)
      : week.commits
  ...
  const totalCommits = weeklyData.reduce((sum: number, w: any) => sum + w.commits, 0)
  const avgPerWeek = Math.round(avgCommits)
  const peakWeek = weeklyData.reduce((max: any, w: any) => w.commits > (max?.commits || 0) ? w : max, { commits: 0 })
  
  const recentTrend = enrichedData.slice(-4).reduce((sum: number, w: any) => sum + w.commits, 0) / 4
  const previousTrend = enrichedData.slice(-8, -4).reduce((sum: number, w: any) => sum + w.commits, 0) / 4
  const trendChange = ((recentTrend - previousTrend) / previousTrend * 100).toFixed(1)
    ...
    <div className="bg-gradient-to-br from-gray-900/50 to-gray-950/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6 hover:border-cyan-500/40 transition-all duration-300">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">Commit patterns and trends over time</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <div className="p-3 bg-gray-800/30 rounded-lg border border-gray-700/50">
          ...
          <p className={`text-xl font-bold ${parseFloat(trendChange) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="h-80 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={enrichedData} margin={{ top: 20, right: 20, left: 0, bottom: 40 }}>
            <defs>
            ...
            <YAxis 
              ...
              axisLine={{ stroke: '#374151' }}
              tickLine={{ stroke: '#374151' }}
              label={{ value: 'Commits', angle: -90, position: 'insideLeft', fill: '#9CA3AF', fontSize: 12 }}
            />
