import { ProcessedExperience, MonthData, ProcessedTimelineData } from './types'
...
export const processTimelineData = (
  ...
  const earliestStart = allExperiences.length > 0
...
export const getCompanyColor = (companyName: string, type: 'work' | 'volunteer'): string => {
  const colors =
    type === 'work'
...
export const arrangeExperiences = (allExperiences: ProcessedExperience[]) => ({
  ...
  volunteerExperiences: allExperiences.filter((exp) => exp.type === 'volunteer'),
})
