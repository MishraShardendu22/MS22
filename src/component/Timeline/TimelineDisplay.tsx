...
import { TimelineDisplayProps, ProcessedExperience, ExperiencePosition } from './types'
import { processTimelineData, arrangeExperiences } from './utils'
...
export const TimelineDisplay = ({ experiences, volunteerExperiences: volunteerExpProps }: TimelineDisplayProps) => {
  ...
  const getExperiencePosition = (exp: ProcessedExperience): ExperiencePosition => {
  ...
  useEffect(() => {
    if (!scrollContainerRef.current || processedData.months.length === 0) return
    ...
    requestAnimationFrame(() => {
      ...
      const monthWidth = isMobile ? 80 : 120
      const scrollTo = currentMonthIndex !== -1
        ? Math.max(0, currentMonthIndex * monthWidth - container.clientWidth / 2 + 60)
  ...
  const { workExperiences, volunteerExperiences } = arrangeExperiences(processedData.allExperiences)
